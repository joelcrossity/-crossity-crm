...
/* ------------------------------------------------------------------
   ...
   Dos piezas chicas que van juntas en todos los formularios largos: el
   ofrecimiento de recuperar lo que quedó a medias, y el aviso de que se
   está guardando solo.

   El aviso no ocupa lugar cuando no hay nada que decir: un renglón fijo
   que dice "limpio" es ruido permanente para tranquilizar una vez.
   ------------------------------------------------------------------ */
...
export function AvisoBorrador({
  ...
  return (
    <div
      role="status"
      className="surge flex flex-wrap items-center gap-3 rounded-md border border-amarillo
                 bg-amarillo-aire px-3 py-2"
    >
      <span className="text-2xs text-tinta">
        Quedó algo a medio cargar de la vez pasada.
      </span>
      <button type="button" onClick={alRestaurar} className="boton boton-principal boton-chico">
        Recuperarlo
      </button>
      <button
        type="button"
...
export function EstadoDelBorrador({ estado }: { estado: 'limpio' | 'guardando' | 'guardado' }) {
  if (estado === 'limpio') return null
  return (
    <span className="flex items-center gap-1.5 text-2xs text-gris-50">
      <span
        className={`size-1.5 rounded-full ${
          estado === 'guardando' ? 'bg-amarillo' : 'bg-verde'
        }`}
        aria-hidden
      />
      {estado === 'guardando' ? 'Guardando…' : 'Guardado en esta máquina'}
    </span>
  )
}
